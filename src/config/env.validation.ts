...
const baseEnvValidationSchema = Joi.object({
  ...
  // SMTP / Mail
  SMTP_SERVICE: Joi.string().allow('').optional(),
  SMTP_HOST: Joi.string().when('NODE_ENV', {
    is: 'production',
    then: Joi.when('MAIL_HOST', {
      is: Joi.string().min(1),
  ...
  MAIL_DEFAULT_EMAIL: Joi.string().allow('').optional(),
  MAIL_DEFAULT_NAME: Joi.string().allow('').optional(),
  // Accepts both plain email ("a@b.com") and RFC 5322 display-name format ("Name <a@b.com>")
  SMTP_FROM_EMAIL: Joi.string().when('NODE_ENV', {
    is: 'production',
    then: Joi.when('MAIL_DEFAULT_EMAIL', {
      is: Joi.string().min(1),
      then: Joi.string().allow('').optional(),
      otherwise: Joi.required(),
    }),
